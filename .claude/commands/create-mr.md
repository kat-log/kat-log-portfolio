...
### D. マージ後のローカル同期（PR作成後に確認）

PR 作成後、以下を確認：

**マージが完了しましたか？ [ターゲットブランチ名] に切り替えてリモートの変更を取り込みますか？**

- `Y` / `y` / `yes` → 以下を実行:
  ```bash
  git checkout [ターゲットブランチ名] && git pull origin [ターゲットブランチ名]
  ```
- `N` / `n` / `no` → 現在のブランチのまま終了

※ [ターゲットブランチ名] は PR 作成時に指定したベースブランチ（デフォルト: main）
